 ...
 * sensors.cpp
 *
 * Mock sensor implementation — animates dashboard values smoothly for
 * UI development and demos. Replace sensors_update() body (or add
 * #ifdef MOCK_SENSORS guards) when integrating real CAN/OBD hardware.
 *
 * Animation sequence:
...
{
    // Seed static/stable values
    g_dash.speed_mph        = 0;
    g_dash.rpm              = 800;   // idle
    g_dash.gear             = 0;     // Neutral
    ...
    ESP_LOGI(STAG, "Mock sensor init complete");
}
...
void sensors_update(void)
{
    ...
    // Apply with light smoothing (IIR filter, alpha=0.15)
    ...
    // Coolant warms up from cold start, stabilizes at 195°F
    if (g_dash.coolant_temp_f < 195) {
        g_dash.coolant_temp_f += 1;
    }

    // Small tire pressure variation at speed (cosmetic)
